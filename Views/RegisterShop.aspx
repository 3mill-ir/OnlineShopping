<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Main.master" AutoEventWireup="true" CodeFile="RegisterShop.aspx.cs" Inherits="Views_RegisterShop" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
    <title><%="ثبت نام فروشگاه" + " | " + Constants.StaticTitle%></title>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder_UnderMenu" Runat="Server">
</asp:Content>
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <div class="ContentMainDiv" style="padding : 15px;">
        <div class="EmptyList">
            این صفحه در دست طراحی می باشد.
        </div>
    </div>
</asp:Content>
<asp:Content ID="Content4" ContentPlaceHolderID="ContentPlaceHolder_Foot" Runat="Server">
</asp:Content>

